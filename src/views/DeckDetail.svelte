  ...
  async function gerarLinkCompartilhamento() {
    try {
      const { error } = await supabase
        ...
        .eq("id", baralhoLocal.id);
      if (error) throw error;

      const link = `${window.location.origin}/?share=${baralhoLocal.id}`;
      await navigator.clipboard.writeText(link);
      alert("Link de compartilhamento copiado! Envie para seus amigos.");
    } catch (erro) {
      alert("Erro ao compartilhar: " + erro.message);
    }
  }
...
>
  <div class="text-center sm:text-left w-full sm:w-auto">
    <span
      class="text-xs font-bold uppercase tracking-wider flex items-center justify-center sm:justify-start gap-1 {$modoEscuro
        ? 'text-indigo-400'
